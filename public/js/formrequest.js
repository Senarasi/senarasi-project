...
function addItem() {
    ...
    // Update select element if NCS is selected
    var selectElements = document.querySelectorAll(".freqSelect");
    selectElements.forEach(function (selectElement) {
        selectElement.addEventListener("change", function () {
            var selectedOption = this.options[this.selectedIndex].value;
            var nameCell = this.parentNode.nextElementSibling;
            if (selectedOption === "NCS") {
                nameCell.innerHTML = `
                        <select id="name" class="form-select request-form">
                            <option>Choose One</option>
                            <option>Option 1</option>
                        </select>`;
            } else {
                nameCell.innerHTML = '<input type="text" class="tablecoba" />';
            }
        });
    });
}
...
function addBundleItems() {
    ...
    divContainer.innerHTML = `
            ...
            <div class="mb-3  fw-bold">
                ...
                <table class="table table-hover" style="font: 300 16px Narasi Sans, sans-serif; width: 100%; margin-top: 12px; margin-bottom: 12px; text-align: center">
                    ...
                    <tbody id="tableBody">
                        <tr>
                            <th scope="row" id="autonumber" style="padding-top: 16px">1</th>
                            <td style="text-align: start">
                                <select class="form-select request-form" >
                                    <option>Choose One</option>
                                    <option>Production Crews</option>
                ...
                <button type="button" class="button-departemen" onclick="addSubDescription()" style="font-size: 14px" >
                    Add Sub Description
    ...
    // // Salin elemen ke tab lain
    // var otherTableContainer = document.getElementById('data2-tab-pane').querySelector('#tableContainer');
    // otherTableContainer.appendChild(divContainer.cloneNode(true));
    ...
    selectElements.forEach(function (selectElement) {
        selectElement.addEventListener("change", function () {
            ...
            if (selectedOption === "NCS") {
                nameCell.innerHTML =
                    '<select id="name" class="form-select" style="background-color: transparent; border-radius: 0px; border: none; border-bottom: 1px solid #4a25aa"><option>Choose One</option><option>Option 1</option></select>';
            } else {
                nameCell.innerHTML = '<input type="text" class="tablecoba" />';
            }
        });
...
function deleteItem(event) {
    ...
    var row = deleteButton.closest("tr"); // Mencari baris (row) terdekat
    var table = row.closest("table"); // Mendapatkan tabel terkait

    row.remove(); // Menghapus baris dari tabel

    // Memperbarui nomor secara otomatis setelah penghapusan
    updateNumbers(table);
}

function updateNumbers(table) {
    // Ambil semua elemen <th> yang memiliki atribut scope="row" dari tabel terkait
    var thElements = table.querySelectorAll('th[scope="row"]');

    // Lakukan iterasi untuk memperbarui nomor secara otomatis
    thElements.forEach(function (th, index) {
        th.textContent = index + 1; // Nomor dimulai dari 1
    });
}
