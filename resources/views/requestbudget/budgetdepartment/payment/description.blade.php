...
@section('title')
    Create Payment Request - Budgeting System
@endsection
@section('costum-css')
<style>
    .button-coy {
...
@endsection
@section('content')
    <a href="{{ route('request-budget-department.payment.edit', $departmentRequestPayment->department_request_payment_id ) }}" class="navback" style="text-decoration: none;">
    ...
    <ul class="nav nav-tabs" id="myTab" role="tablist" style="margin-bottom: 24px; font: 600 18px Narasi Sans, sans-serif; ">
        <li class="nav-item" role="presentation"  style="flex: 1; text-align: center;">
            <button class="nav-link  tablinks" id="data5-tab" data-url="{{ route('request-budget-department.payment.edit', $departmentRequestPayment->department_request_payment_id ) }}"
                data-bs-toggle="tab" data-bs-target="#data5-tab-pane" type="button" role="tab"
                aria-controls="data1-tab-pane" aria-selected="false" style="width: 100%; color: #4a25aa; letter-spacing: 0.5px; font-weight: 300" >GENERAL</button>
        </li>

        <li class="nav-item" role="presentation"  style="flex: 1; text-align: center;">
            <button class="nav-link active tablinks" id="preview-tab" data-url="{{ route('request-budget-department.payment.description', $departmentRequestPayment->department_request_payment_id) }}"
                data-bs-toggle="tab" data-bs-target="#preview-tab-pane" type="button" role="tab"
                aria-controls="preview-tab-pane" aria-selected="true" style="width: 100%; letter-spacing: 0.5px;">DESCRIPTION
            </button>
        </li>
    ...
    <div class="tab-content" id="myTabContent" style="margin-top: 24px">
            ...
            <table class="table table-hover align-middle"
                ...
                <thead style="font-weight: 500; border-top: 1px solid #c4c4c4">
                    <tr class="dicobain">
                        ...
                        <th scope="col-2" style="width: 30%">Amount</th>
                        <td scope="col " style="border-left:1px solid #c4c4c4; vertical-align: middle; width: 15%;">Action</td>
                    </tr>
                </thead>
                <tbody>
                    ...
                    <tr>
                        {{-- <th scope="row ">1</th> --}}
                        <td style="text-align: start">{{ $item->description }}
                            @if ($item->note != null)
                                <span style="font-weight: 300; color:rgb(173, 173, 173); padding-left: 8px">( {{ $item->note }} )</span>
                                @endif
                        </td>

                        <td style="position: relative; text-align: end; font-weight: 300; ">
                        ...
                        <td  style="border-left:1px solid #c4c4c4">
                            <span style="display: flex; gap: 8px; justify-content: center" >
                                <a href="javascript:;" style="font-size: 14px" data-bs-toggle="modal" data-bs-target="#edit-payment-item-{{ $item->department_request_payment_item_id}}"
                                    data-id=""
                                    data-url="">
                                ...
                                <form class="form-delete" onsubmit="return confirm('Are you sure want to delete this data?')"
                                    action="{{ route('request-budget-department.payment.description.destroy', $item->department_request_payment_item_id)}}" method="POST">
                                    @csrf
                                    @method('DELETE')
                        ...
                        @include('requestbudget.budgetdepartment.payment.modal_edit_description', ['data' => $item])
                    </tr>

                    @empty
                    <tr>
                        <td colspan="3">Item is empty</td>
                    </tr>
                    @endforelse
                    <tr style="border-bottom: 1px solid #c4c4c4; ">
                        <td colspan="1" class="text-center" style="font-weight: 500; background-color: #dbdee8">
                            Total</td>
                            <td style="position: relative; text-align: end; font-weight: 400; ">
                                <span style="float: left;">Rp</span>
        ...
        <div class="d-flex" style="justify-content: space-between; margin-left:12px; margin-right:12px;">
            <button type="button" class="button-coy me-3" onclick="addItem()" data-bs-toggle="modal"
                data-bs-target="#add-payment-item">Add Item<svg xmlns="http://www.w3.org/2000/svg" width="18" height="18"
            ...
            <div>
                @if ($departmentRequestPayment->departmentRequestPaymentItems->isNotEmpty())
                <form method="POST" action="{{ route('request-budget-department.payment.submit' , $departmentRequestPayment->department_request_payment_id) }}">
                    @csrf
                    @method('PATCH')
...
@section('modal')
    <div class="modal justify-content-center fade" id="add-payment-item" data-bs-keyboard="false" tabindex="-1"
        aria-labelledby="staticBackdropLabel" aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered modal-lg">
            <div class="modal-content">
                <div class="modal-body bg-white">
                    <form action="{{ route('request-budget-department.payment.description.store')}}" method="POST" class="modal-form-check"
                        ...
                        <div class="row">
                            <div class="col mb-3">
                                    ...
                                    <input type="text" class="form-control" id="desc" name="description"  style="padding: 8px" oninput="this.value = this.value.toUpperCase()" required/>
                            </div>

                        </div>

                        <div class="mb-3">
                            <label for="formatted_amount" class="form-label">Amount</label>
                            <input type="text" class="form-control" id="formatted_amount" name="formatted_amount" style="padding: 8px" />
                            <input type="hidden" id="amount" name="amount" class="@error('amount') is-invalid @enderror" />
                            @error('amount')
                        ...
                        <div class="col">
                            <div class="mb-3">
                                ...
                                <textarea type="text"  class="form-control" id="note" name="note" cols="30" rows="2" oninput="this.value = this.value.toUpperCase()"></textarea>
                            </div>
                        </div>
...
<script>
    var formattedAmoutInput = document.getElementById('formatted_amount');
    var amountInput = document.getElementById('amount');

    formattedAmoutInput.addEventListener('keyup', function(e) {
        var formattedBudget = formatRupiah(this.value, 'Rp');
        formattedAmoutInput.value = formattedBudget; // Update the input field with the formatted value

        // Set the raw numeric value to the hidden input
        amountInput.value = parseAmount(formattedBudget);
    });
    ...
    function parseAmount(formattedBudget) {
